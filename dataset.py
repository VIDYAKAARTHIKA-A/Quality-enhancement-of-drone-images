...
class mydata(Dataset):
    def __init__(self, LR_path, GT_path, in_memory=True, transform=None):
        ...
        if in_memory:
            self.LR_img = [self.load_image(os.path.join(self.LR_path, lr)) for lr in self.LR_img]
            self.GT_img = [self.load_image(os.path.join(self.GT_path, gt)) for gt in self.GT_img]
            
            # Remove None values caused by corrupt images
            self.LR_img, self.GT_img = zip(*[(lr, gt) for lr, gt in zip(self.LR_img, self.GT_img) if lr is not None and gt is not None])
        else:
            self.LR_img = [lr for lr in self.LR_img if self.is_image_valid(os.path.join(self.LR_path, lr))]
            self.GT_img = [gt for gt in self.GT_img if self.is_image_valid(os.path.join(self.GT_path, gt))]
...
class testOnly_data(Dataset):
    def __init__(self, LR_path, in_memory=True, transform=None):
        ...
        if in_memory:
            self.LR_img = [self.load_image(os.path.join(self.LR_path, lr)) for lr in self.LR_img]
            self.LR_img = [lr for lr in self.LR_img if lr is not None]
        else:
            self.LR_img = [lr for lr in self.LR_img if self.is_image_valid(os.path.join(self.LR_path, lr))]
